...
struct CoffeeShop{
    ...
    let coffeeType = Coffee.allCases
    let cafeMenu: [Coffee] = Coffee.allCases
    ...
    mutating func order(coffee: Coffee, customer: inout Person){ // 주문시 missKim 가격 다운 & 야곰벅스 매출액 증가
        ...
        if customer.money < coffeePrice{
            ...
            makeCoffee(coffee: coffee, barista: barista, customer: customer)
            
        }
        
    }
    
    mutating func makeCoffee(coffee: Coffee, barista: String, customer: Person){
        print("\(coffee)는 바리스타 \(barista)님이 만들고 있습니다!")
...
var yagombukcs = CoffeeShop(cafeName: "yagombucks", cafeProfit: 100000, pickUpTable: "카운터", barista: misterLee.name)

yagombukcs.order(coffee: .americano, customer: &missKim)
yagombukcs.order(coffee: .vanillaLatte, customer: &seok)
